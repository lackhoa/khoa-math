...
def kenum(node: Union[Mole, KSet], root: Mole, orig: LogNode):
    if root.complexity > glob.MAX_CPLEX:
        orig.log('We\'re reached the maximum level of complexity')
        return
    ...
    if type(node) == KSet:
        ...
        for well_formed in form_p(node=node, root=root, orig=orig.sub()):
            ...
            for relationed in cycle_rel_p(
                    node=well_formed, rels=rels, root=root, orig=this_wf_orig.sub()):
                ...
                for finished in fin_p(node=relationed, root=root, orig=this_rel_orig.sub()):
...
def form_p(node, root, orig):
    ...
    cons = list(node['_cons'] & KSet(cons_dic[only(node['_types'])].keys()))
    ...
    recur_num_key  = lambda c: _recur_num(type_ = node_type, con = c)
...
def rel_p(node, rel, orig):
    ...
    if rel.type == 'FUN':
        ...
        for res in _fun_rel(node=node, rel=rel, orig=orig):
            yield res
    elif rel.type == 'UNION':
        orig.log('It\'s a union relation')
        for res in _uni_rel(node=node, rel=rel, orig=orig):
            yield res


def cycle_rel_p(node, rels, orig):
    for new_node, unchecked_rels in repeat_rel_p(
            node=node, rel_iter=iter(rels), orig=orig):
        ...
        if new_node == node:
            ...
            if unchecked_rels:
                ...
                for res in cycle_rel_p(
                        node=new_node, rels=unchecked_rels, orig=new_orig):
                    yield res
            else:
...
def repeat_rel_p(node, rel_iter, orig):
    ...
    try:
        for new_node in rel_p(
                node=node, rel=this_rel, orig=orig):
...
def _fun_rel(node, rel, orig):
    ...
    legit_ins = [kenum(node = node[role], orig = orig.sub())
                 for role in in_roles]
...
def _uni_rel(node, rel, orig):
    ...
    try:
        for uni_legit in kenum(
                node=node[super_role], orig=orig.sub()):
            rc = node.clone()
            ...
            legit_subs = (
                kenum(node=rc[sub_role], orig=orig.sub())\
        ...
        sub_nodes_legit = (
                kenum(node=node[r], orig=orig.sub()) for r in subs_role)
...
def fin_p(node, orig):
    ...
    m_children_enum = [kenum(node=node[key], orig=orig.sub())
